...
import { parseJsonSchemaSpec } from "@/lib/exporters";
import { FieldDefinition } from "@/lib/mock-generator";

interface JsonSchemaImportModalProps {
  onClose: () => void;
...
export function JsonSchemaImportModal({ onClose, onImportSuccess }: JsonSchemaImportModalProps) {
  ...
  const processJson = (rawContent: string) => {
    try {
      setError(null);
      const parsed = parseJsonSchemaSpec(rawContent);
      onImportSuccess(parsed);
      onClose();
    } catch (err: any) {
      setError(`Failed to parse JSON Schema: ${err.message || "Invalid JSON Schema v4/v7 syntax"}`);
    }
  };
      ...
      <div className="bg-mb-surface border border-mb-border rounded-xl max-w-lg w-full p-6 space-y-6 shadow-2xl">
        <div className="flex items-center justify-between border-b border-mb-border pb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-mb-text">Import JSON Schema (v4/v7/2020-12)</h2>
          </div>
        ...
        <p className="text-xs text-mb-text-secondary leading-relaxed">
          Drop any JSON Schema <code className="text-mb-text font-mono">*.json</code> file or paste <code className="text-mb-text font-mono">$schema</code> specification objects.
        </p>
        ...
        >
          ...
          <p className="text-xs font-medium text-mb-text">Drag & drop your JSON Schema file here</p>
          ...
          <input
            ...
            className="hidden"
            id="jsonschema-file-input"
          />
          <label
            htmlFor="jsonschema-file-input"
        ...
        {/* Raw JSON Schema Paste Textarea */}
        <div className="space-y-2">
          <label className="block text-2xs font-mono text-mb-text-tertiary uppercase">Or Paste JSON Schema Object</label>
          <textarea
            value={jsonText}
            onChange={(e) => setJsonText(e.target.value)}
            placeholder='{ "$schema": "http://json-schema.org/draft-07/schema#", "title": "Order", "type": "object", "properties": { "id": { "type": "string", "format": "uuid" }, "total": { "type": "number" } } }'
            rows={5}
