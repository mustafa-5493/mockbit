...
import { parseWireMockMapping } from "@/lib/exporters";
import { FieldDefinition } from "@/lib/mock-generator";

interface WireMockImportModalProps {
  onClose: () => void;
...
export function WireMockImportModal({ onClose, onImportSuccess }: WireMockImportModalProps) {
  ...
  const processJson = (rawContent: string) => {
    try {
      setError(null);
      const parsed = parseWireMockMapping(rawContent);
      onImportSuccess(parsed);
      onClose();
    } catch (err: any) {
      setError(`Failed to parse WireMock mapping: ${err.message || "Invalid WireMock mapping JSON"}`);
    }
  };
      ...
      <div className="bg-mb-surface border border-mb-border rounded-xl max-w-lg w-full p-6 space-y-6 shadow-2xl">
        <div className="flex items-center justify-between border-b border-mb-border pb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-mb-text">Import WireMock JSON Stub Mapping</h2>
          </div>
        ...
        <p className="text-xs text-mb-text-secondary leading-relaxed">
          Drop any WireMock stub mapping <code className="text-mb-text font-mono">*.json</code> file or paste stub JSON to auto-populate endpoint fields.
        </p>
        ...
        >
          ...
          <p className="text-xs font-medium text-mb-text">Drag & drop your WireMock stub mapping file here</p>
          ...
          <input
            ...
            className="hidden"
            id="wiremock-file-input"
          />
          <label
            htmlFor="wiremock-file-input"
        ...
        <div className="space-y-2">
          <label className="block text-2xs font-mono text-mb-text-tertiary uppercase">Or Paste WireMock Stub JSON</label>
          <textarea
            value={jsonText}
            onChange={(e) => setJsonText(e.target.value)}
            placeholder='{ "request": { "method": "POST", "url": "/api/v1/payments" }, "response": { "status": 200, "jsonBody": { "id": "pay_99", "amount": 99.0 } } }'
            rows={5}
