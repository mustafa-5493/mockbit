...
import { parsePostmanCollection } from "@/lib/exporters";
...
export function PostmanImportModal({ onClose, onImportSuccess }: PostmanImportModalProps) {
  ...
  const processJson = (rawContent: string) => {
    try {
      setError(null);
      const parsed = parsePostmanCollection(rawContent);
      onImportSuccess(parsed);
      onClose();
    } catch (err: any) {
      setError(`Failed to parse Postman collection: ${err.message || "Invalid Postman Collection v2.1 JSON"}`);
    }
  };
      ...
      <div className="bg-mb-surface border border-mb-border rounded-xl max-w-lg w-full p-6 space-y-6 shadow-2xl">
        <div className="flex items-center justify-between border-b border-mb-border pb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-mb-text">Import Postman Collection v2.1</h2>
          </div>
        ...
        <p className="text-xs text-mb-text-secondary leading-relaxed">
          Drop any <code className="text-mb-text font-mono">*.postman_collection.json</code> file or paste collection JSON to auto-populate endpoint fields.
        </p>
        ...
        >
          ...
          <p className="text-xs font-medium text-mb-text">Drag & drop your Postman collection file here</p>
        ...
        <div className="space-y-2">
          <label className="block text-2xs font-mono text-mb-text-tertiary uppercase">Or Paste Collection JSON</label>
          <textarea
            value={jsonText}
            onChange={(e) => setJsonText(e.target.value)}
            placeholder='{ "info": { "name": "Stripe Orders" }, "item": [...] }'
            rows={5}
